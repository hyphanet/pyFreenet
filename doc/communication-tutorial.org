...
- [ ] Add interactive shell option: announce / watchme
- [ ] Add function to check whether the other one already added trust (if
      the id publishes a trust list)
- [ ] Get captchas directly, if WoT does not have an identity
...
- [ ] Add a statusline
- [ ] Add a limited number of freebee-CAPTCHAs: Identities randomly
  choose to publish clear KSKs so a limited number of IDs can announce
  ...
  and you disappear thanks to the WoT.
...
- [ ] --autosetup-freenet-if-missing: If Java is available but there’s no freenet node at --host H --port P, just setup a node in ~/.local/var/babcom/ using an approach like scripts/test-autoupgrade
