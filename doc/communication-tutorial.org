...
for an interactive shell. You can copy this verbatim into any new
system. For your own project can replace this skelleton with any other
setup. It provides the constraints of general interactive programs to
ensure that the code examples from this tutorial can be adapted easily
for other projects.
...
And now that the basics are in place, let us start with Freenet stuff.
...
To use these, we’ll simply create fast upload and download functions:
