...
# site
# pkg
from freenet_passlib_170.handlers.misc import plaintext
from freenet_passlib_170.utils import unix_crypt_schemes, to_unicode
from freenet_passlib_170.utils.compat import uascii_to_str, unicode, u
...
class ldap_md5(_Base64DigestHelper):
    ...
    The :meth:`~freenet_passlib_170.ifc.PasswordHash.hash` and :meth:`~passlib.ifc.PasswordHash.genconfig` methods have no optional keywords.
    """
    name = "ldap_md5"
    ident = u("{MD5}")
    _hash_func = md5
...
class ldap_plaintext(plaintext):
    ...
    This class acts much like the generic :class:`!freenet_passlib_170.hash.plaintext` handler,
    except that it will identify a hash only if it does NOT begin with the ``{XXX}`` identifier prefix
    used by RFC2307 passwords.

    The :meth:`~freenet_passlib_170.ifc.PasswordHash.hash`, :meth:`~passlib.ifc.PasswordHash.genhash`, and :meth:`~passlib.ifc.PasswordHash.verify` methods all require the
...
##        from freenet_passlib_170.hosts import host_context
