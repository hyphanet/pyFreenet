"""freenet_passlib_170.crypto.digest -- crytographic helpers used by the password hashes in passlib
...
from freenet_passlib_170.utils import join_bytes, to_native_str, join_byte_values, to_bytes, \
                          SequenceMixin
from freenet_passlib_170.utils.compat import irange, int_types, unicode_or_bytes_types, PY3
...
def _get_hash_aliases(name):
    ...
    if name.startswith("scram-"): # helper for SCRAM protocol (see freenet_passlib_170.handlers.scram)
        name = name[6:]
...
def _get_hash_const(name):
    ...
    try:
        ...
        const.__doc__ = ("wrapper for hashlib.new(%r),\n"
                         "generated by freenet_passlib_170.crypto.digest.lookup_hash()") % name
        return const

    # use builtin md4 as fallback when not supported by hashlib
    if name == "md4":
        from freenet_passlib_170.crypto._md4 import md4
        return md4
...
def lookup_hash(digest, return_unknown=False):
    ...
    :param return_unknown:
        By default, this function will throw an :exc:`~freenet_passlib_170.exc.UnknownHashError` if no hash constructor
...
def pbkdf2_hmac(digest, secret, salt, rounds, keylen=None):
    ...
    .. versionchanged:: 1.7
        ...
        * builtin pure-python backend
        ...
        See :data:`freenet_passlib_170.crypto.digest.PBKDF2_BACKENDS` to determine
...
if PY3 and _force_backend in ["any", "from-bytes"]:
    ...
    from struct import Struct
    from freenet_passlib_170.utils import sys_bits
    ...
    def _get_pbkdf2_looper(digest_size):
        ...
        # compile helper
        #
        code = compile(source, "<generated by freenet_passlib_170.crypto.digest._get_pbkdf2_looper()>", "exec")
    ...
    from freenet_passlib_170.utils import int_to_bytes
