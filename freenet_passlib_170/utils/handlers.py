"""freenet_passlib_170.handler - code for implementing handlers, and global registry for handlers"""
#=============================================================================
# imports
#=============================================================================
...
# site
# pkg
import freenet_passlib_170.exc as exc, freenet_passlib_170.ifc as ifc
from freenet_passlib_170.exc import MissingBackendError, PasslibConfigWarning, \
                        PasslibHashWarning
from freenet_passlib_170.ifc import PasswordHash
from freenet_passlib_170.registry import get_crypt_handler
...
from freenet_passlib_170.utils.binary import (
    BASE64_CHARS, HASH64_CHARS, PADDED_BASE64_CHARS,
    HEX_CHARS, UPPER_HEX_CHARS, LOWER_HEX_CHARS,
    ALL_BYTE_VALUES,
)
from freenet_passlib_170.utils.compat import join_byte_values, irange, u, native_string_types, \
                                 uascii_to_str, join_unicode, unicode, str_to_uascii, \
                                 join_unicode, unicode_or_bytes_types, PY2, int_types
from freenet_passlib_170.utils.decor import classproperty, deprecated_method
# local
__all__ = [
...
def guess_app_stacklevel(start=1):
    ...
    looks for first frame not part of freenet_passlib_170.
    """
    ...
    try:
        while frame:
            ...
            if not name.startswith("freenet_passlib_170.") or name.startswith("passlib.tests."):
                return max(1, count)
            count += 1
            frame = frame.f_back
        return start
...
class HasManyIdents(GenericHandler):
    ...
    def using(cls,  # keyword only...
        ...
        """
        This mixin adds support for the following :meth:`~freenet_passlib_170.ifc.PasswordHash.using` keywords:
...
class BackendMixin(PasswordHash):
    ...
    .. classmethod:: _load_backend_{name}(dryrun=False)
        ...
        :raises freenet_passlib_170.exc.PasslibSecurityError:
    ...
    def get_backend(cls):
        ...
        :raises freenet_passlib_170.exc.MissingBackendError:
...
class PrefixWrapper(object):
    ...
    this is used mainly by the :doc:`ldap crypt <freenet_passlib_170.hash.ldap_crypt>` handlers;
    such as :class:`~freenet_passlib_170.hash.ldap_md5_crypt` which wraps :class:`~passlib.hash.md5_crypt` and adds a ``{CRYPT}`` prefix.
